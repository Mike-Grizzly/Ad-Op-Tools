...
type Props = {
  ...
  generateAction: (input: unknown) => Promise<{ data: { generated_url: string }; error?: undefined } | { data?: undefined; error: string }>
  saveTemplateAction: (input: unknown) => Promise<{ data: { id: string }; error?: undefined } | { data?: undefined; error: string }>
}

function buildURL(base: string, source: string, medium: string, campaign: string, content: string, term: string): string {
        ...
        <div style={{ position: 'absolute', zIndex: 30, top: 'calc(100% + 5px)', left: 0, right: 0, background: '#fff', border: '1px solid #e1e4e9', borderRadius: 11, boxShadow: '0 12px 30px -8px rgba(22,25,34,.22)', padding: 5, maxHeight: 210, overflow: 'auto' }}>
          {options.map((opt) => (
            <button
              ...
              style={{ display: 'block', width: '100%', border: 'none', background: 'transparent', textAlign: 'left', padding: '8px 10px', borderRadius: 7, fontSize: 13.5, color: '#161922', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}
            >
              {opt.value}
            </button>
          ))}
...
export function UTMForm({ templates, onGenerated, onTemplateSaved, onCopy, generateAction, saveTemplateAction }: Props) {
  ...
  const generatedUrl = buildURL(baseUrl, source, medium, campaign, content, term)
  ...
  async function handleGenerate() {
    ...
    const result = await generateAction({ base_url: baseUrl, source, medium, campaign, content: content || undefined, term: term || undefined })
  ...
  return (
    <section style={{ background: '#fff', border: '1px solid #e9ebef', borderRadius: 16, padding: 24, boxShadow: '0 1px 2px rgba(22,25,34,.04)' }}>
      ...
      <div style={{ marginBottom: 20, padding: '13px 14px', background: '#f7f8fa', border: '1px solid #ebedf1', borderRadius: 11 }}>
        ...
        <div style={{ position: 'relative' }}>
          <select
            ...
            style={{ width: '100%', appearance: 'none', padding: '10px 36px 10px 12px', border: '1px solid #e1e4e9', borderRadius: 9, fontSize: 14, fontFamily: 'inherit', fontWeight: 500, color: '#161922', background: '#fff', outline: 'none', cursor: 'pointer' }}
          >
          ...
          <svg width="16" height="16" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: '#9aa0aa' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="m6 9 6 6 6-6" />
          </svg>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input
          value={campaign}
          onChange={(e) => setCampaign(e.target.value.slice(0, 200))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 22 }}>
        <div>
          <label style={{ display: 'block', fontSize: 12.5, fontWeight: 600, color: '#4b5563', marginBottom: 6 }}>
            Content <span style={{ color: '#9aa0aa', fontWeight: 500 }}>· optional</span>
          </label>
          <input value={content} onChange={(e) => setContent(e.target.value)} placeholder="carousel_a" style={inputStyle} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: 12.5, fontWeight: 600, color: '#4b5563', marginBottom: 6 }}>
            Term <span style={{ color: '#9aa0aa', fontWeight: 500 }}>· optional</span>
          </label>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <div style={{ position: 'relative', background: '#0f1118', border: '1px solid #1d2030', borderRadius: 12, padding: 15, fontFamily: 'var(--font-mono)', fontSize: 12.5, lineHeight: 1.75, wordBreak: 'break-all', minHeight: 58, color: generatedUrl ? '#cdd3e0' : '#5b6273' }}>
      ...
      {savingTemplate && (
        <div style={{ marginBottom: 16, padding: '14px', background: '#f7f8fa', border: '1px solid #ebedf1', borderRadius: 11 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="e.g. Google Search — Brand"
              autoFocus
            ...
            <button
              ...
              disabled={!templateName.trim() || savePending}
              style={{ padding: '10px 16px', border: 'none', borderRadius: 9, background: '#4f46e5', color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', cursor: templateName.trim() && !savePending ? 'pointer' : 'not-allowed', opacity: !templateName.trim() || savePending ? 0.6 : 1 }}
            >
            ...
            <button
              ...
              style={{ padding: '10px 12px', border: '1px solid #e1e4e9', borderRadius: 9, background: '#fff', color: '#4b5563', fontSize: 13, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}
            >
              Cancel
            </button>
          </div>
      ...
      <div style={{ display: 'flex', gap: 11 }}>
        <button
          ...
          disabled={!generatedUrl || urlInvalid || generating}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 12, border: 'none', borderRadius: 10, background: '#4f46e5', color: '#fff', fontSize: 14, fontWeight: 700, fontFamily: 'inherit', cursor: !generatedUrl || urlInvalid || generating ? 'not-allowed' : 'pointer', opacity: !generatedUrl || urlInvalid || generating ? 0.6 : 1 }}
        >
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '12px 16px', border: '1px solid #e1e4e9', borderRadius: 10, background: '#fff', color: '#4b5563', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', cursor: savingTemplate ? 'not-allowed' : 'pointer' }}
        >
