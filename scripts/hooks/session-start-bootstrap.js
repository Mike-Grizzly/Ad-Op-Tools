 ...
 *   2. Current working directory (project-level install)
 *   3. ~/.claude (direct install)
 *   4. Several well-known plugin sub-paths under ~/.claude/plugins/
 *   5. Versioned cache directories under ~/.claude/plugins/cache/
...
function resolvePluginRoot() {
  ...
  // Check project root first (project-level install)
  const cwd = process.cwd();
  if (hasRunnerRoot(cwd)) {
    return cwd;
  }
